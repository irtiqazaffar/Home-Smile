...
const Staff = () => {
  ...
  const { can } = usePermission("staff");

  const { data: roles } = useAsync(RoleAndPermissionServices.getAllRoles);
            ...
            >
              ...
              <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                <Select onChange={(e) => setRole(e.target.value)}>
                  <option value="All" defaultValue hidden>{t("StaffRole")}</option>
                  {roles.map(role => (
      ...
      {loading ? (
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          <Table>
            ...
            <StaffTable staffs={dataTable} lang={lang} roles={roles} />
          </Table>
          <TableFooter>
            <Pagination
