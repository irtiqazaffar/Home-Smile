...
const useFilter = (data) => {
  ...
  const serviceData = useMemo(() => {
    ...
    if (departmentType) {
      services = services.filter(
        (search) =>
          search?.name[lang]
            ?.toLowerCase()
            ?.includes(departmentType?.toLowerCase()) ||
          search?.description[lang]?.toLowerCase().includes(departmentType?.toLowerCase())
      );
    }
    ...
    if (brandType) {
      services = services.filter(
        (search) =>
          ...
          search?.description[lang]?.toLowerCase().includes(brandType?.toLowerCase())
      );
    }
    ...
    if (ratingType) {
      services = services.filter(
        (search) =>
          search?.product?.title[lang]
            ?.toLowerCase()
            ?.includes(ratingType?.toLowerCase()) ||
          search?.customer?.name?.toLowerCase().includes(ratingType?.toLowerCase())
      );
    }

    //admin Filtering
    if (role) {
      services = services.filter((staff) => staff.role._id === role);
    }
    ...
    if (currency) {
      services = services.filter((cur) =>
        cur.name[lang].toLowerCase().includes(currency.toLowerCase())
      );
    }
  ...
  const handleSubmitDepartment = (e) => {
    e.preventDefault();
    setDepartmentType(departmentRef.current.value);
  };
  ...
  const handleSubmitRating = (e) => {
    e.preventDefault();
    setRatingType(ratingRef.current.value);
  };
