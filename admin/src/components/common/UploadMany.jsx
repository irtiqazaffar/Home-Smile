...
const UploadMany = ({
  ...
  const handleExportCSV = () => {
    const exportDataFromAPI = async (fileName, dataProcessingCallback) => {
      try {
        ...
        if (fileName === "products") {
          ...
          data = dataProcessingCallback ? dataProcessingCallback(res.products) : res.products;
        } else {
          data = exportData;
        }
        ...
        setLoadingExport({ name: "", status: false });
        setDropDown(false);
        // Handle error (optional: log or display an error message)
      }
    };

    if (location.pathname === "/products") {
      // Data transformation for products
      exportDataFromAPI("products", (products) =>
        products
          .filter((product) => product?.status !== "hide")
          .map((product) => ({
            id: product?._id,
            status: product?.status,
            title: product?.title?.en.slice(0, 145),
            description: product?.description?.en.slice(0, 190),
            availability: product?.stock > 0 ? 'in_stock' : 'out_of_stock',
            link: `https://Home Smile.com/product/${product?.slug}`,
            image_link: product?.image[0],
            price: `${product?.prices?.originalPrice}.00 INR`,
            sale_price: `${product?.prices?.price}.00 INR`,
      ...
      exportDataFromAPI("categories");
    } else if (location.pathname === "/attributes") {
      exportDataFromAPI("attributes");
    } else if (location.pathname === "/coupons") {
      exportDataFromAPI("coupons");
    } else if (location.pathname === "/customers") {
      exportDataFromAPI("customers");
    }
  };
