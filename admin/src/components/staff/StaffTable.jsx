...
const StaffTable = ({ staffs, lang, roles }) => {
  const {
    title,
    serviceId,
    handleModalOpen,
  ...
  const { can, editButtonCLick, deleteButtonCLick } = usePermission("staff", handleUpdate, handleModalOpen);
  ...
  return (
    <>
      ...
      <MainDrawer>
        <StaffDrawer id={serviceId} roles={roles}/>
      </MainDrawer>
      ...
      <TableBody>
        {staffs?.map((staff) => (
          <TableRow key={staff._id}>
            ...
            <TableCell>
              <span className="text-sm font-semibold">{showingTranslateValue(staff?.role?.name)}</span>
            </TableCell>
