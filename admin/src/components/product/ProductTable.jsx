...
const ProductTable = ({ products, isCheck, setIsCheck }) => {
  ...
  const { can, editButtonCLick, deleteButtonCLick } = usePermission("product", handleUpdate, handleModalOpen);
        ...
        {products?.map((product, i) => (
          <TableRow key={i + 1}>
            ...
            <TableCell className="text-center">
              <ToggleStatusButton id={product._id} status={product.status} canToggleStatus={can.edit}/>
            </TableCell>
            <TableCell>
              <EditDeleteButton
                id={product._id}
