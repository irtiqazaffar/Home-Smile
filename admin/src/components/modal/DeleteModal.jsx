...
const DeleteModal = ({ id, ids, setIsCheck, category, title, useParamId }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      setIsSubmitting(true);
      if (location.pathname === "/roles-and-permission") {
        if (ids) {
          const res = await RoleAndPermissionServices.deleteManyRole({
            ids: ids,
          });
          setIsUpdate(true);
          notifySuccess(res.message);
          ...
          const res = await RoleAndPermissionServices.deleteRole(id);
          setIsUpdate(true);
          notifySuccess(res.message);
          setServiceId();
          closeModal();
      ...
      if (location.pathname === "/departments") {       
        if (ids) {
          const res = await DepartmentService.deleteManyDepartment({
            ids: ids,
          });
          setIsUpdate(true);
          notifySuccess(res.message);
      ...
      if (location.pathname === "/brands") {
        if (ids) {
          const res = await BrandServices.deleteManyBrand({
            ids: ids,
          });
          setIsUpdate(true);
          notifySuccess(res.message);
      ...
      if (location.pathname === "/ratings") {
        if (ids) {
          const res = await RatingServices.deleteManyRating({
            ids: ids,
          });
          setIsUpdate(true);
          notifySuccess(res.message);
          ...
          const res = await RatingServices.deleteRating(id);
          setIsUpdate(true);
          notifySuccess(res.message);
          setServiceId();
          closeModal();
