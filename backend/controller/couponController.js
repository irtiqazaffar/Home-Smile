...
const getUserCoupons = async (req, res) => {
  try {
    ...
    // Get all coupon IDs where isUsed is false
    const couponIds = customer.coupons
      .filter(c => c.isUsed === false)
      .map(c => c.coupon);
    ...
    // Filter coupons with `couponAll === "Ok"`
    const couponsWithOk = Coupons.filter(coupon => coupon.couponAll === "Ok");

    const combinedCoupons = [...allCoupons, ...couponsWithOk];

    // console.log("User Coupon ", allCoupons);
    // console.log("All Coupon ", couponsWithOk);

    // Combine or respond with results as needed
    res.send({
      withOk: combinedCoupons,
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const updateCoupon = async (req, res) => {
  try {
    ...
    if (coupon) {
      ...
      coupon.couponAll = req.body.couponAll;
