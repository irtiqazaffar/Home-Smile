...
const sendEmailToAdmin = async (req, res) => {
  try {
      const { email, name, subject, message, location, number } = req.body
      // console.log("Email ", req.body);
      const contactUsEmailBody = `
        <div style="font-family: 'Arial', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #f9f9f9;">
          <h2 style="background-color: black; color: white; padding: 10px; text-align: center; border-radius: 10px 10px 0 0;">New Query</h2>
          <div style="padding: 20px;">
            ...
            <p style="font-size: 16px; color: #555;"><strong>Number:</strong> ${number}</p>
            <p style="font-size: 16px; color: #555;"><strong>Location:</strong> ${location}</p>
            ...
            <div style="text-align: center; margin-top: 30px;">
              <a href="mailto:${email}" style="background-color: black; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Reply to ${name}</a>
            </div>
          </div>
