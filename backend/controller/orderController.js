...
const getAllOrders = async (req, res) => {
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select(
        "_id invoice paymentMethod subTotal total user_info deliveryDate discount shippingCost status isCancelByCustomer createdAt updatedAt"
      )
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limits);
...
const updateOrder = (req, res) => {
  ...
  // Conditionally add 'status' and 'deliveryDate' to the update object
  if (status) {
  ...
  // If there's nothing to update, send a response immediately
