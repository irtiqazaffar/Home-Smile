...
const getCategoriesFromDepartments = async (req, res) => {
  try {
    const departments = await Department.find().select("_id name");

    const departmentIds = departments.map((dept) => dept._id);

    const categories = await Category.find({ departmentId: { $in: departmentIds }, parentId: { $ne: null }, status: "show" })
      .sort({ _id: -1 });
      

    const departmentList = departments.map((department) => {
      const associatedCategories = categories.filter(
        (category) => category.departmentId._id.toString() === department._id.toString()
      );

      // Return the department with its associated categories as children
      return {
        _id: department._id,
        name: department.name,
        children: associatedCategories.map((category) => ({
          _id: category._id,
          name: category.name,
...
const readyToParentAndChildrenCategory = (categories, parentId = null) => {
  ...
  for (let cate of Categories) {
    categoryList.push({
      ...
      // departmentId: cate.departmentId,
