...
const getProductById = async (req, res) => {
  try {
    const product = await Product.findById(req.params.id)
      ...
      .populate({ path: "brand" });
      console.log("Single Data From ", product);
      res.send(product);
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const getShowingSuggestProducts = async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(400).send({ message: "The 'data' field cannot be empty" });
    }
...
const updateProduct = async (req, res) => {
  ...
  // console.log('Update Single ', req.body)
  try {
    const product = await Product.findById(req.params.id);
...
const getShowingStoreProducts = async (req, res) => {
  try {
    ...
    const { category, title, slug, query, page } = req.query;
    ...
    if (query && query != "latest") {
      ...
      queryObject.$or = [
        ...languageCodes.map((lang) => ({
          [`description`]: queryRegex, // Match query in titles in various languages
