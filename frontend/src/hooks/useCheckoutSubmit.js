...
const useCheckoutSubmit = (storeSetting) => {
  ...
  const { isEmpty, emptyCart, items, removeItem, addItem, cartTotal } = useCart();
  ...
  const submitHandler = async (data) => {
    ...
    // return;
    let codAvailableItems = items.filter(item => item.isCodAvaialble === true) || [];
    let codNotAvailableItems = items.filter(item => item.isCodAvaialble === false) || [];
    ...
    try {
      ...
      const { amount, id, currency } =
        await OrderServices.createOrderByRazorPay({
          amount: Math.round(discountedTotal).toString(),
        });
          ...
          handler: async function (response) {
            ...
            if (res) {
              ...
              await NotificationServices.addNotification({
                message: `${data?.firstName} placed $${discountedTotal} order!`,
                orderId: res._id,
  ...
  const handleCouponCode = async (e) => {
    ...
    try {
      ...
      if (total < result[0]?.minimumAmount) {
        ...
        const maxCouponDiscountPercent = Number(globalSetting.max_coupon_discount_percent) || 5;
        const maxCouponDiscountAmount = Number(globalSetting.max_coupon_discount_amount) || 500;
        const percentAmount = (cartTotal * (result[0]?.discountType?.value || maxCouponDiscountPercent)) / 100;
        ...
        notifySuccess(
          `Your Coupon ${result[0].couponCode} is Applied!`
        );
