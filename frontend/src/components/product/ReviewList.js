    ...
    <div className="border-b border-gray-300 py-4">
      <div className="flex flex-col lg:flex-row justify-between mb-2">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-sm text-gray-900">by {review?.customer?.name || "MSS Customer"}</span>
        </div>
...
const ReviewList = ({ reviews }) => {
  return (
    <div className="w-full mx-auto p-6 mt-4 bg-white rounded-lg">
      ...
      {/* If no reviews */}
      {reviews.length === 0 ? (
        <p className="text-gray-800">No reviews yet. Be the first to leave one!</p>
      ) : (
        <div>
