      ...
      <div className="w-full max-h-full">
        ...
        {error ? (
          ...
          <div className="relative grid gap-2 p-6">
            {/* Conditionally render departments or normal categories */}
            {type === "department" ? (
              departments?.map((department) => (
                <div key={department._id} hidden={department.children.length <= 0}>
                  {/* Department header with expand/collapse toggle */}
                  <div
                    className="flex justify-between items-center cursor-pointer font-semibold font-serif text-sm px-6 py-2 border-b"
                  ...
                  {/* Render department's categories if expanded */}
