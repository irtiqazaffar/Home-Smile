...
const Coupon = ({ couponInHome }) => {
  ...
  const { data, error } = useAsync(() => CouponServices.getUserCoupons(user?.id));
  ...
  const mergedArray = [
    ...new Map(
      [...(data?.withOk || []), ...(data?.datas || [])].map(item => [item._id, item])
    ).values(),
  ];
  ...
  const maxCouponDiscountAmount = Number(globalSetting.max_coupon_discount_amount) || 500;
  ...
  return (
    <>
      {couponInHome ? (
        data?.withOk?.slice(0, 2).map((coupon) => (
          <div
            key={coupon._id}
                  ...
                  <h6 className="pl-1 text-base font-medium text-gray-600">
                    <span className="text-lg md:text-xl lg:text-xl text-red-500 font-bold">
                      {coupon?.discountType?.type === "fixed" ? (
                        <span>{coupon?.discountType?.value}{" "}Off</span>
                      ) : (
                        <span>{coupon?.discountType?.value}%{" Off "}Upto {maxCouponDiscountAmount}</span>
                      )}
                    </span>
                  </h6>
                  <div className="ml-2">
        ...
        data?.withOk?.length > 0 ? (
          data?.withOk?.map((coupon) => (
            <div
              key={coupon._id}
              ...
              <div className="tengah p-6 flex items-center justify-items-start">
                ...
                <div className="ml-5">
                  ...
                  <h2 className="pl-1 text-base font-medium text-gray-600">
                    <span className="text-lg md:text-xl lg:text-xl text-red-500 font-bold">
                      {coupon?.discountType?.type === "fixed" ? (
                        <span>{globalSetting?.default_currency || "$"}{coupon?.discountType?.value}{" "}Off</span>
                      ) : (
                        <span>{coupon?.discountType?.value}%{" Off "}{"Upto "}{globalSetting?.default_currency || "$"}{maxCouponDiscountAmount}</span>
                      )}
                    </span>                      
                  </h2>
                </div>
          ...
          <p className="col-span-2 flex flex-col justify-center align-middle items-center m-auto text-xl">
            <span className="text-4xl mb-2 text-emerald-500">🥹 Oops...</span>
            <span className="text-xl text-gray-700">Coupons not available</span>
            <span className="text-xl text-green-700"><Link href="/search">Continue Shopping →</Link></span>
          </p>       
        )
      )}
    </>
